...
#La fonction calculer_factorielle calcule la factorielle toute seule 
#car elle peut marcher avec un seul chiffre et non pas 2 comme les autres operateurs
...
def appliquer_operateur(a, b, op):
    if op=="!":
        return calculer_factorielle(a) # seulement a car il n<y a qu'un seul chiffre dans l'operation
    if op == "+":
        return a + b
    if op == "-":
        return a - b
        ...
        if est_nombre(token):
            ...
            if token == "!":
                if len(nombres) < 1:
                    return "#"
                a = nombres.pop()
                b = 0  # On met 0 par défaut pour ne pas laisser b vide
            else:
                if len(nombres) < 2:
...
def tests_calculer_formule():
    ...
    # Factorielle
    assert calculer_formule("5 !") == 120
    assert calculer_formule("0 !") == 1
    assert calculer_formule("3 2 + !") == 120 # (3 + 2)! = 120
